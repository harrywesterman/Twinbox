...
def test_jitsi_platform_overlay_provides_broker_secret_sync_service_and_ingress():
    ...
    assert re.search(
        r"ghcr\.io/harrywesterman/twinbox-manager-worker:jitsi-openid-sha-[0-9a-f]{7}",
        deployment_text,
    )
...
def test_jitsi_openid_image_is_pinned_and_patched_for_room_scoped_short_lived_tokens():
    ...
    assert "type=sha,prefix=jitsi-openid-sha-" in workflow_text
    ...
    assert 'image_tag="jitsi-openid-sha-${GITHUB_SHA:0:7}"' in workflow_text
