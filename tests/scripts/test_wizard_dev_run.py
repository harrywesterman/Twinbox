...
def test_wizard_dev_run_loads_default_local_config_and_uses_tty_remote_run(tmp_path: Path):
    ...
    assert calls[0] == {
        "cmd": "ssh",
        "args": ["root@proxmox-dev", "mkdir -p '/root/twinbox-dev'"],
    }
    assert calls[1] == {
        "cmd": "scp",
        "args": [
    ...
    assert calls[2] == {
        ...
        "args": [
            "-tt",
            "root@proxmox-dev",
            "chmod +x '/root/twinbox-dev/setup-wizard.sh' && bash '/root/twinbox-dev/setup-wizard.sh'",
        ],
    }
...
def test_wizard_dev_run_cli_overrides_config_file_defaults(tmp_path: Path):
    ...
    assert proc.returncode == 0, proc.stderr
    calls = _read_remote_log(tmp_path)
    assert calls[0]["args"] == ["root@override-host", "mkdir -p '/tmp/wizard-dev'"]
    assert calls[1]["args"] == [
        str(REPO_ROOT / "wizard" / "setup-wizard.sh"),
    ...
    assert calls[2]["args"] == [
        "-tt",
        "root@override-host",
        "chmod +x '/tmp/wizard-dev/setup-wizard.sh' && bash '/tmp/wizard-dev/setup-wizard.sh'",
    ]
...
def test_wizard_dev_run_requires_target_when_no_config_or_override_is_present(tmp_path: Path):
    ...
    assert "Set WIZARD_DEV_SSH_TARGET in .env.wizard.local or pass --target." in proc.stderr
