...
- `install-trivy-operator` installs Trivy Operator so every running container image is continuously scanned for known CVEs and the results are published as in-cluster `VulnerabilityReport` objects that Grafana can visualise.
