...
- `install-postgres-clusters` deploys all database clusters defined under `gitops/databases/`. Each cluster gets a CloudNativePG `Cluster`, PgBouncer `Pooler` resources (read-write and read-only), a `ScheduledBackup` for daily snapshots, and an `ExternalSecret` that pulls credentials from OpenBao. The current `authentik-db` manifest runs a single instance; the reusable template under `gitops/databases/_template/` keeps three instances for future application databases. The step also seeds the Authentik database credentials into OpenBao early so the Authentik database can bootstrap before `install-authentik-idp` renders the rest of the Authentik secret. Applications connect through the pooler service (e.g. `authentik-db-pooler-rw.databases.svc.cluster.local`).
