...
- `provision-nodes` keeps control-plane VMs fixed at `4 GB RAM / 10 GB disk`, sizes workers from an `80%` share of the free disk budget across the three Proxmox hosts with a worker-disk slider, and applies the `twinbox.io/role=worker` label to the nodes that should host Longhorn.
