...
| `PROXMOX_FILE_DATASTORE` | `local` | Datastore for Talos disk-image uploads; Talos provisioning requires Proxmox `import` content here, while the greenfield setup wizard may also enable `snippets` for the Management VM cloud-init file |
...
| Variable | Example | Description |
|----------|---------|-------------|
| `TWINBOX_IMAGE_TAG` | `sha-fab771b` | GHCR image tag for all three manager services |
