...
8. The separate `install-argocd` follow-up installs Argo CD and applies the bootstrap root Application from `gitops/argocd/bootstrap/root.yaml`. That root Application is intentionally limited to safe non-secret workloads such as `whoami` and `headlamp`; anything that depends on Vaultwarden-backed material should stay behind the ESO/Bitwarden path.
...
- If `install-argocd` runs, Argo CD is installed as a separate follow-up after ESO/Bitwarden and the bootstrap root Application is applied from `gitops/argocd/bootstrap/root.yaml`. The root Application only manages safe non-secret workloads like `whoami` and `headlamp` so secret-bearing workloads stay out of the initial Argo sync set.
