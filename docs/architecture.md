...
- Route-layer secrets such as `traefik-dashboard-auth` and `wiredoor-gateway` are also projected from Vaultwarden via per-namespace `SecretStore`/`ExternalSecret` resources.
