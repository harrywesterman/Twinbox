...
2. `provision-nodes` sizes control-plane VMs at `4 GB RAM / 10 GB disk`, sizes workers from a `100%` share of the free disk budget across the three Proxmox hosts with a worker-disk slider, and labels Talos nodes with `twinbox.io/role`.
...
13. `install-alloy` installs Grafana Alloy as the shared collection pipeline for logs, events, and traces.
