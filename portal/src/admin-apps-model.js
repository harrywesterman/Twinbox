...
function normalizeBundleCard(bundle = {}, cardsById = new Map()) {
  const bundleCards = Array.isArray(bundle.apps)
    ? bundle.apps.map((appId) => cardsById.get(String(appId))).filter(Boolean)
    : [];
  ...
  return {
    ...
    iconUrl:
      bundle.iconUrl ||
      bundle.iconArtworkUrl ||
      sourceCard?.iconUrl ||
      sourceCard?.iconArtworkUrl ||
      "",
    iconAlt: bundle.iconAlt || `${bundle.title || "Bundle"} icon`,
    iconText:
      bundleCards[0]?.iconText ||
      bundleCards[0]?.title?.slice(0, 2).toUpperCase() ||
      String(bundle.title || "GB")
        .slice(0, 2)
        .toUpperCase(),
    status,
    app_state: status,
...
export function buildAdminAppsViewModel({ catalog = {}, query = "", selectedAppId = "" } = {}) {
  ...
  const bundles = Array.isArray(catalog?.bundles)
    ? catalog.bundles.map((bundle) => normalizeBundleCard(bundle, cardsById))
    : [];
  const filteredCards = cards
    .filter((card) => matchesQuery(card, normalizedQuery))
    .sort(compareCardsByTitle);
  const filteredBundles = bundles.filter((bundle) => matchesBundleQuery(bundle, normalizedQuery));
  const selectedApp =
    filteredCards.find((card) => card.id === selectedAppId) ||
