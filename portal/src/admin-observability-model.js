...
function buildDefaultProfile(profileId, source = {}) {
  const defaults = {
    minimal: {
      ...
      description: 'Keep metrics, remove the log/trace/dashboard stack, and avoid Longhorn-backed observability storage.',
      accent: '#14b8a6',
      priority: 'low',
    },
    full: {
    ...
    off: {
      ...
      description: 'Prune the observability applications and their storage to keep the cluster quiet.',
      accent: '#ef4444',
...
export function buildObservabilityViewModel({
  ...
  const rawProfiles = observabilityConfig?.profiles && typeof observabilityConfig.profiles === 'object'
    ? observabilityConfig.profiles
    : {};

  const profiles = ['minimal', 'full', 'off'].map((profileId) => buildDefaultProfile(profileId, rawProfiles[profileId]));
  ...
  return {
    ...
    description: trimString(observabilityConfig.description) || 'Choose how much monitoring the cluster should carry.',
    footnote: trimString(observabilityConfig.footnote) || 'Metrics-server stays enabled in every mode so kubectl top keeps working.',
