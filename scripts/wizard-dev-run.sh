...
usage() {
  cat <<EOF
Usage: $(basename "$0") [--target root@host] [--remote-dir /path]
...
upload_and_run_remote() {
  ...
  ssh "${SSH_TARGET}" "mkdir -p ${quoted_remote_dir}"
  scp "${LOCAL_WIZARD_PATH}" "${SSH_TARGET}:${REMOTE_SCRIPT_PATH}"
  ssh -tt "${SSH_TARGET}" "chmod +x ${quoted_remote_script} && bash ${quoted_remote_script}"
}
