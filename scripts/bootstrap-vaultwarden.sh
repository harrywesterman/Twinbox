...
VAULTWARDEN_DOMAIN="${VAULTWARDEN_DOMAIN:-http://127.0.0.1:${VAULTWARDEN_LOCAL_PORT}}"
VAULTWARDEN_SERVER_URL="${VAULTWARDEN_SERVER_URL:-$VAULTWARDEN_DOMAIN}"
...
VAULTWARDEN_PUBLIC_URL="${VAULTWARDEN_PUBLIC_URL:-$VAULTWARDEN_DOMAIN}"

export BITWARDENCLI_APPDATA_DIR="$BW_APPDATA_DIR"

wait_for_vaultwarden() {
  local local_url="${VAULTWARDEN_PUBLIC_URL}"
  local attempt=""
