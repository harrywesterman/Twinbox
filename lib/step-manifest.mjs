...
export function normalizeStepManifest(manifest, file, categoryId) {
  ...
  if (manifest.depends_on !== undefined && !Array.isArray(manifest.depends_on)) {
  ...
  return {
    ...
    depends_on: Array.isArray(manifest.depends_on)
      ? manifest.depends_on.map((dependency) => String(dependency))
      : [],
    runner: {
