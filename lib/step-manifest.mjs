...
export function normalizeInputDefinition(input, file) {
  ...
  const normalized = {
    ...
    secret_field: typeof input.secret_field === "string" ? input.secret_field.trim() : undefined,
    visible_when:
      input.visible_when && typeof input.visible_when === "object"
        ? {
            input: trimString(input.visible_when.input),
            equals: String(input.visible_when.equals ?? ""),
          }
        : undefined,
  };

  if (normalized.sensitive && !normalized.secret_field) {
    throw new Error(`sensitive input ${input.id} requires secret_field in ${file}`);
  }
