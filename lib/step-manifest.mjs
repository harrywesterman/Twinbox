...
function normalizeDashyItem(rawItem, file, stepId, index) {
  ...
  if (!urlTemplate && !outputUrlKey) {
    throw new Error(`dashy item ${index + 1} for ${stepId} must define url_template or output_url_key in ${file}`);
  }
  if (urlTemplate && outputUrlKey) {
    throw new Error(`dashy item ${index + 1} for ${stepId} must not define both url_template and output_url_key in ${file}`);
  }
