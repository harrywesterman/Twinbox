...
export function backupHosts({ nodes, storages, networks, cluster }) {
  return nodes
    ...
    .map((node) => {
      ...
      const files = storages.filter((s) => s.node === name && active(s));
      ...
      return {
        ...
        error: cluster.bridge && !bridge ? `Bridge ${cluster.bridge} is unavailable` : !disks.length ? "No active VM datastore" : "",
      };
    })
