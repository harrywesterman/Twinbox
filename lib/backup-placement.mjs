  ...
  return nodes
    ...
    .map((node) => {
      ...
      const files = storages
        .filter((s) => s.node === name && active(s))
        .sort((a, b) => a.storage.localeCompare(b.storage));
      const fileCandidates = files.filter((s) => content(s, "iso") || content(s, "snippets"));
      const preferred = cluster.file_datastore || cluster.metadata?.file_datastore;
      const file = files.find((s) => s.storage === preferred) || fileCandidates[0] || files[0];
