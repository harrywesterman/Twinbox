...
export function itemPrefix(runtimeEnv = process.env) {
  const env = normalizeRuntimeEnv(runtimeEnv);
  return trimString(env.TWINBOX_SECRET_ITEM_PREFIX)
    || "twinbox";
}
...
export function globalSecretPath(runtimeEnv, item) {
  return path.join(secretRoot(runtimeEnv), "global", `${safePathComponent(item, "secret item")}.json`);
}
...
export function clusterSecretPath(runtimeEnv, clusterId, item, attachment) {
  return path.join(clusterSecretDir(runtimeEnv, clusterId, item), safePathComponent(attachment, "attachment"));
}
...
export function resolveSecretRecordPath(runtimeEnv, ref, context = {}) {
  ...
  if (scope === "cluster") {
    const clusterId = safePathComponent(ref?.cluster_id || context.clusterId || context.cluster_id, "cluster id");
...
export function readItemRecord(runtimeEnv, ref, context = {}) {
  ...
  if (scope === "cluster") {
    const clusterId = safePathComponent(ref?.cluster_id || context.clusterId || context.cluster_id, "cluster id");
    return readJsonFileIfExists(path.join(secretRoot(runtimeEnv), "cluster", clusterId, item, "metadata.json"));
  }
...
export function writeItemRecord(runtimeEnv, ref, value, context = {}) {
  ...
  if (scope === "cluster") {
    const clusterId = safePathComponent(ref?.cluster_id || context.clusterId || context.cluster_id, "cluster id");
    writeJsonFile(path.join(secretRoot(runtimeEnv), "cluster", clusterId, item, "metadata.json"), record);
    return path.join(secretRoot(runtimeEnv), "cluster", clusterId, item, "metadata.json");
  }
...
export function listItemObjects(runtimeEnv, search, context = {}) {
  ...
  if (record) {
    const attachmentDir = normalized.scope === "cluster"
      ? clusterSecretDir(runtimeEnv, normalized.cluster_id || context.clusterId || context.cluster_id, normalized.item)
      : path.join(secretRoot(runtimeEnv), "global", safePathComponent(normalized.item, "secret item"));
...
export function downloadSecretAttachment(runtimeEnv, attachmentName, itemId, outputFile, session, context = {}) {
  const ref = {
