...
export class SecretBroker {
  ...
  findItem(ref, context = {}) {
    ...
    let item = items.find((candidate) => String(candidate?.name || "") === itemName);
    if (item) {
    ...
    if (this.backend === "vaultwarden") {
      this.refreshSession();
      const refreshedItems = listBitwardenItems(this.runtimeEnv, itemName, session);
      item = refreshedItems.find((candidate) => String(candidate?.name || "") === itemName);
      if (item) {
        this.itemCache.set(itemName, {
          value: item,
          expiresAt: Date.now() + this.cacheTtlMs,
  ...
  upsertAttachment(rawRef, sourceFile, context = {}) {
    ...
    if (existingAttachment?.id) {
      deleteBitwardenAttachment(this.runtimeEnv, existingAttachment.id, item.id, session);
    }
