...
function readClusterMetadata(runtimeEnv, ref, context = {}) {
  ...
  return readJsonFileIfExists(path.join(clusterSecretDir(runtimeEnv, clusterId, ref.item), "metadata.json"));
}
...
export class SecretBroker {
  ...
  resolveTextRef(rawRef, context = {}) {
    ...
    if (!value) {
      throw new Error(`secret field not found: ${ref.field} on ${buildItemName(this.runtimeEnv, ref, context)}`);
    }
  ...
  materializeRef(rawRef, label, context = {}) {
    const ref = normalizeSecretRef(rawRef);
    const tempRoot = this.runtimeEnv.TWINBOX_SECRET_TEMP_DIR || path.join(this.runtimeEnv.MANAGER_DATA_DIR || "/tmp", "twinbox-secrets");
