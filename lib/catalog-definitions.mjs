...
import { normalizeCategoryManifest, normalizeStepManifest } from "./step-manifest.mjs";
...
function loadReferencedAppStep({
  categoriesRoot,
  categoryId,
  categoryFile,
  reference,
...
function loadCategorySteps({
  ...
  if (fs.existsSync(stepsRoot)) {
    const stepDirs = fs
      .readdirSync(stepsRoot, { withFileTypes: true })
...
export function loadCatalogDefinitions({
  ...
  const categoriesRoot =
    process.env.TWINBOX_CATEGORIES_DIR || path.join(workspaceRoot, "categories");
  const response = {
    categoriesRoot,
    categories: [],
    steps: [],
  ...
  for (const categoryDir of categoryDirs) {
    ...
    try {
      ...
      response.errors.push(
        error instanceof Error ? error.message : `failed to load ${categoryFile}`
      );
    }
  }
    ...
    if (fs.existsSync(bundlesRoot)) {
      ...
      for (const bundleFile of bundleFiles) {
        try {
          ...
          response.errors.push(
            error instanceof Error ? error.message : `failed to load ${bundleFile}`
          );
        }
      }
