...
import { STEP_ICON_MANIFEST } from "../manager-web/src/assets/step-icons/manifest.js";
...
const WIZARD_ICON_FILE_BASE_BY_STEP_ID = new Map(
  STEP_ICON_MANIFEST.map((entry) => [entry.stepId, entry.fileBase]),
);

const WIZARD_ICON_DATA_URL_BY_STEP_ID = new Map();
...
function readWizardIconDataUrl(stepId) {
  if (WIZARD_ICON_DATA_URL_BY_STEP_ID.has(stepId)) {
    return WIZARD_ICON_DATA_URL_BY_STEP_ID.get(stepId);
  }

  const fileBase = WIZARD_ICON_FILE_BASE_BY_STEP_ID.get(stepId);
  if (!fileBase) {
    WIZARD_ICON_DATA_URL_BY_STEP_ID.set(stepId, "");
    return "";
  }

  const svgPath = new URL(`../manager-web/src/assets/step-icons/${fileBase}.svg`, import.meta.url);
  ...
  WIZARD_ICON_DATA_URL_BY_STEP_ID.set(stepId, dataUrl);
  return dataUrl;
}

function resolveDashyIcon(step, dashyItem, zoneName) {
  const wizardIcon = readWizardIconDataUrl(step.id);
  if (wizardIcon) {
    return wizardIcon;
  }
