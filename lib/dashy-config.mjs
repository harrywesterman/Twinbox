...
function wizardIconUrl(zoneName, fileBase) {
  return zoneName ? `https://webwizard.${zoneName}/assets/step-icons/${fileBase}.svg` : "favicon";
}

function localStepIconPaths(workspaceRoot, fileBase, extension) {
  if (!workspaceRoot || !fileBase) {
    return [];
  }
...
function resolveLocalStepIconReference(workspaceRoot, fileBase) {
  const normalizedBase = trimString(fileBase);
  if (!normalizedBase) {
    return "";
  }

  for (const extension of ["svg", "png"]) {
    for (const filePath of localStepIconPaths(workspaceRoot, normalizedBase, extension)) {
...
const DASHY_OFFICIAL_ICON_BY_STEP = new Map([
  ...
  ["install-copyparty", "install-copyparty"],
  ...
  ["provision-netbird-bastion", "provision-netbird-bastion"],
]);

const DASHY_OFFICIAL_ICON_BY_TITLE = new Map([
  ["Proxmox", "install-management-consoles"],
  ["Forgejo", "forgejo"],
...
function resolveArtwork(value, zoneName, workspaceRoot = "") {
  ...
  const localIcon = resolveLocalStepIconReference(workspaceRoot, normalized);
  if (localIcon) {
    return localIcon;
  }
...
function resolveDashyIcon(step, dashyItem, zoneName, workspaceRoot = "") {
  ...
  if (titleIcon) {
    return resolveArtwork(titleIcon, zoneName, workspaceRoot);
  }

  const stepIcon = DASHY_OFFICIAL_ICON_BY_STEP.get(step.id);
  if (stepIcon) {
    return resolveArtwork(stepIcon, zoneName, workspaceRoot);
  }

  if (/^(?:install|configure|provision)-[a-z0-9-]+$/i.test(explicitIcon)) {
    return resolveArtwork(explicitIcon, zoneName, workspaceRoot);
  }

  return resolveArtwork("install-dashy-dashboard", zoneName, workspaceRoot) || "favicon";
}
...
export function buildDashyConfig({
  steps,
  stepStateById,
  cluster,
  excludeStepIds = [],
  ...
  for (const step of sortedSteps) {
    ...
    step.dashy.items.forEach((dashyItem, index) => {
      ...
      appendSectionItem(sectionMap, dashyItem.section, {
        ...
        url,
        icon: resolveDashyIcon(step, dashyItem, zoneName, workspaceRoot),
  ...
  for (const fixedItem of FIXED_DASHY_ITEMS) {
    appendSectionItem(sectionMap, fixedItem.section, {
      ...
      url: fixedItem.url,
      icon: resolveArtwork(fixedItem.icon, zoneName, workspaceRoot) || fixedItem.icon,
