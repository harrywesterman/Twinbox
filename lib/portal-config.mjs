...
function wizardIconUrl(zoneName, fileBase) {
  ...
  if (!zoneName || !normalizedFileBase) {
    return "";
  }

  return `https://twinboxwizard.${zoneName}/assets/step-icons/${normalizedFileBase}.svg`;
}
...
function buildCard({
  ...
  const iconUrl = wizardIconUrl(zoneName, sourceStep?.id || slugify(title));
