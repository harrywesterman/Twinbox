...
function buildUserAdminConfig(content = {}) {
  const rawUserAdmin =
    content?.userAdmin && typeof content.userAdmin === "object" ? content.userAdmin : {};

  return {
    eyebrow: trimString(rawUserAdmin.eyebrow) || "Admin",
    title: trimString(rawUserAdmin.title) || "Gebruikers en groepen",
    description:
      trimString(rawUserAdmin.description) ||
      "Beheer gebruikers en zakelijke groepen vanuit het Twinbox Portal.",
    emptyStateTitle:
      trimString(rawUserAdmin.emptyStateTitle) || "Nog geen beheerbare groepen ingesteld",
    emptyStateDescription:
      trimString(rawUserAdmin.emptyStateDescription) ||
...
function buildObservabilityProfileConfig(profile = {}, defaults = {}) {
  return {
    title: trimString(profile.title) || trimString(defaults.title) || "",
    summary: trimString(profile.summary) || trimString(defaults.summary) || "",
    description: trimString(profile.description) || trimString(defaults.description) || "",
    impact:
      normalizeList(profile.impact).length > 0
        ? normalizeList(profile.impact)
        : normalizeList(defaults.impact),
    keeps:
      normalizeList(profile.keeps).length > 0
        ? normalizeList(profile.keeps)
        : normalizeList(defaults.keeps),
    removes:
      normalizeList(profile.removes).length > 0
        ? normalizeList(profile.removes)
        : normalizeList(defaults.removes),
    footprint: {
      cpu: trimString(profile?.footprint?.cpu) || trimString(defaults?.footprint?.cpu) || "",
      memory:
        trimString(profile?.footprint?.memory) || trimString(defaults?.footprint?.memory) || "",
      storage:
        trimString(profile?.footprint?.storage) || trimString(defaults?.footprint?.storage) || "",
    },
    warning: trimString(profile.warning) || trimString(defaults.warning) || "",
    accent: trimString(profile.accent) || trimString(defaults.accent) || "",
    priority: trimString(profile.priority) || trimString(defaults.priority) || "",
  };
}

function buildObservabilityConfig(content = {}) {
  const rawObservability =
    content?.observability && typeof content.observability === "object"
      ? content.observability
      : {};
  const rawProfiles =
    rawObservability?.profiles && typeof rawObservability.profiles === "object"
      ? rawObservability.profiles
      : {};

  const defaults = {
    minimal: {
      title: "Minimal",
      summary: "Metrics-first monitoring with the smallest possible runtime footprint.",
      description:
        "Keep Kubernetes resource visibility without the logs, traces, and dashboards stack. Prometheus runs with short retention and no Longhorn-backed persistence so the cluster can breathe on small hardware.",
      impact: [
        "Prometheus stays on in a lightweight form",
        "Grafana, Loki, Tempo, and Alloy are removed",
        "Metrics-server remains available for kubectl top",
      ],
      keeps: [
        "Kubernetes resource metrics",
        "Cluster capacity visibility",
        "The smallest supported monitoring baseline",
      ],
      removes: [
        "Grafana dashboards",
        "Log aggregation via Loki",
        "Trace storage via Tempo",
        "Kubernetes log and event shipping via Alloy",
      ],
      footprint: {
        cpu: "~0.2-0.3 core",
        memory: "~256-512 MiB",
      ...
      warning:
        "This mode is optimized for small clusters and trades historical depth for lower churn.",
      accent: "#14b8a6",
      priority: "low",
    },
    full: {
      title: "Full stack",
      summary: "Current observability stack with metrics, logs, traces, and dashboards.",
      description:
        "Keep the existing Twinbox observability layout: Prometheus, Alertmanager, Loki, Tempo, Alloy, and Grafana all active for the richest operator experience.",
      impact: [
        "Prometheus, Alertmanager, Loki, Tempo, Alloy, and Grafana are active",
        "Dashboards, log search, and traces stay available",
        "Best for larger clusters or active debugging",
      ],
      keeps: ["Full Grafana dashboards", "Log and trace exploration", "Alertmanager notifications"],
      removes: [],
      footprint: {
        cpu: "~1.2-1.5 cores",
        memory: "~1.5-2 GiB",
        storage: "~50 GiB observability data",
      },
      warning: "This is the most capable and the heaviest mode.",
      accent: "#2563eb",
      priority: "default",
    },
    off: {
      title: "Off",
      summary: "Disable the observability stack completely.",
      description:
        "Remove the observability applications and keep only the small baseline cluster utilities. This is the most aggressive option for the quietest possible cluster.",
      impact: [
        "Prometheus is removed",
        "Grafana, Loki, Tempo, and Alloy are removed",
        "Only baseline utilities remain",
      ],
      keeps: [
        "Metrics-server stays available",
        "Cluster apps keep running",
        "GitOps and storage remain intact",
      ],
      removes: [
        "Every observability application",
        "All observability PVCs",
        "Historical logs, traces, and dashboards",
      ],
      footprint: {
        cpu: "~0 core beyond baseline utilities",
        memory: "~0 MiB for observability apps",
        storage: "observability PVCs are pruned",
      },
      warning: "Use this only when you want the stack fully gone, not merely trimmed.",
      accent: "#ef4444",
  ...
  return {
    eyebrow: trimString(rawObservability.eyebrow) || "Admin",
    title: trimString(rawObservability.title) || "Observability control",
    description:
      trimString(rawObservability.description) ||
      "Choose a monitoring profile that matches the cluster size and workload.",
    footnote:
      trimString(rawObservability.footnote) ||
      "Metrics-server stays enabled in every mode so kubectl top continues to work.",
    profiles: {
      minimal: buildObservabilityProfileConfig(rawProfiles.minimal, defaults.minimal),
      full: buildObservabilityProfileConfig(rawProfiles.full, defaults.full),
      off: buildObservabilityProfileConfig(rawProfiles.off, defaults.off),
    },
  };
}

function buildAppProfileLookup(content = {}) {
  return content?.appProfiles && typeof content.appProfiles === "object" ? content.appProfiles : {};
}
...
function normalizeMobileLink(entry) {
  if (!entry || typeof entry !== "object") {
    return null;
  }

  const platform = trimString(entry.platform);
  const label = trimString(entry.label);
  const url = trimString(entry.url);
  if (!platform || !label || !/^https?:\/\//i.test(url)) {
    return null;
  }
...
function normalizeMobileLinks(value) {
  return Array.isArray(value)
    ? value.map((entry) => normalizeMobileLink(entry)).filter(Boolean)
    : [];
}
...
function buildCard({
  ...
  const description =
    trimString(resolvedProfile.description) ||
  ...
  const mobileLinks = normalizeMobileLinks(resolvedProfile.mobileLinks);
  const label = trimString(resolvedProfile.label) || title;
  const route = `/apps/${slugify(title)}`;
  const iconUrl = wizardIconUrl(
    zoneName,
    sourceStep?.id || trimString(iconFileBase) || slugify(title)
  );
...
function collectStepCards({
  ...
  const allowedCategories =
    Array.isArray(allowedCategoryIds) && allowedCategoryIds.length > 0
      ? new Set(allowedCategoryIds.map((value) => trimString(value)).filter(Boolean))
      : null;
  const allowedStepIds =
    Array.isArray(visibleStepIds) && visibleStepIds.length > 0
      ? new Set(visibleStepIds.map((value) => trimString(value)).filter(Boolean))
      : null;
...
export function buildPortalConfig({
  ...
  const settings = {
    issueUrl:
      trimString(content?.settings?.issueUrl) ||
      "https://github.com/harrywesterman/Twinbox/issues/new/choose",
    authentikAdminUrl:
      interpolateTemplate(content?.settings?.authentikAdminUrl || "", zoneName, {
        portal_host: portalHost,
        authentik_admin_url: authentikAdminUrl,
        authentik_user_url: authentikUserUrl,
      }) || authentikAdminUrl,
    authentikUserUrl:
      interpolateTemplate(content?.settings?.authentikUserUrl || "", zoneName, {
        portal_host: portalHost,
        authentik_user_url: authentikUserUrl,
      }) || authentikUserUrl,
    authentikOtpUrl:
      interpolateTemplate(content?.settings?.authentikOtpUrl || "", zoneName, {
        portal_host: portalHost,
        authentik_user_url: authentikUserUrl,
      }) || authentikUserUrl,
    languages: [
  ...
  const observability = buildObservabilityConfig(content);
  ...
  const userApps = collectStepCards({
    ...
    visibleStepIds: installedAppIds,
  });

  const adminCards = collectStaticCards(
    Array.isArray(content?.adminLinks)
      ? content.adminLinks
      : [
          {
            title: "Dashy",
  ...
  const hero = {
    ...
    description:
      trimString(content?.hero?.description) ||
      "Open your apps, settings, and status from one place.",
  };
  ...
  return {
    ...
    appSections: [
      {
        name: "Apps",
        items: Array.from(groupedUserApps.get("Apps") || []).sort((left, right) =>
