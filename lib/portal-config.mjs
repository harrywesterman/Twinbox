...
function buildObservabilityProfileConfig(profile = {}, defaults = {}) {
  return {
    ...
    impact: normalizeList(profile.impact).length > 0
      ? normalizeList(profile.impact)
      : normalizeList(defaults.impact),
    keeps: normalizeList(profile.keeps).length > 0
      ? normalizeList(profile.keeps)
      : normalizeList(defaults.keeps),
    removes: normalizeList(profile.removes).length > 0
    ...
    footprint: {
      ...
      memory: trimString(profile?.footprint?.memory) || trimString(defaults?.footprint?.memory) || "",
      storage: trimString(profile?.footprint?.storage) || trimString(defaults?.footprint?.storage) || "",
    },
...
function buildObservabilityConfig(content = {}) {
  const rawObservability = content?.observability && typeof content.observability === "object"
    ? content.observability
    : {};
  const rawProfiles = rawObservability?.profiles && typeof rawObservability.profiles === "object"
  ...
  const defaults = {
    minimal: {
      ...
      description: "Keep Kubernetes resource visibility without the logs, traces, and dashboards stack. Prometheus runs with short retention and no Longhorn-backed persistence so the cluster can breathe on small hardware.",
      impact: [
      ...
      warning: "This mode is optimized for small clusters and trades historical depth for lower churn.",
      accent: "#14b8a6",
      priority: "low",
    },
    full: {
      ...
      description: "Keep the existing Twinbox observability layout: Prometheus, Alertmanager, Loki, Tempo, Alloy, and Grafana all active for the richest operator experience.",
      impact: [
    ...
    off: {
      ...
      description: "Remove the observability applications and keep only the small baseline cluster utilities. This is the most aggressive option for the quietest possible cluster.",
      impact: [
  ...
  return {
    ...
    description: trimString(rawObservability.description)
      || "Choose a monitoring profile that matches the cluster size and workload.",
    footnote: trimString(rawObservability.footnote)
...
function collectStepCards({
  ...
  const allowedCategories = Array.isArray(allowedCategoryIds) && allowedCategoryIds.length > 0
    ...
    : null;
  const allowedStepIds = Array.isArray(visibleStepIds) && visibleStepIds.length > 0
