...
kubectl exec -n nextcloud deploy/nextcloud -c nextcloud -- su -s /bin/bash www-data -c \
  "php occ config:app:set eurooffice DocumentServerUrl --value='https://nextcloud-eurooffice.${public_zone_name}/' --type=string &&
   ...
   php occ config:app:set eurooffice sameTab --value='false' --type=string &&
   php occ config:app:set eurooffice enableSharing --value='true' --type=string &&
   php occ config:app:set eurooffice preview --value='false' --type=string"
