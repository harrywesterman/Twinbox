...
delete_group_binding() {
  ...
  if [[ -n "$existing_pk" ]]; then
    authentik_api_write DELETE "/policies/bindings/${existing_pk}/" >/dev/null
  fi
}
...
pixelfed_secret_file="$(mktemp "${TMPDIR:-/tmp}/pixelfed-bootstrap-XXXXXX")"
pixelfed_rendered_manifest="$(mktemp "${TMPDIR:-/tmp}/pixelfed-application-XXXXXX")"
...
if [[ -n "$admins_group_id" ]]; then
  delete_group_binding "$application_uuid" "$admins_group_id"
fi
...
bash "$WORKSPACE_ROOT/scripts/manager/ensure-netbird-service.sh" \
  --service-name "pixelfed" \
  --service-domain "pixelfed.${public_zone_name}" \
  --service-path /
