...
resource "tls_private_key" "dashy_signing" {
  algorithm = "RSA"
  rsa_bits  = 4096
}

resource "tls_self_signed_cert" "dashy_signing" {
  private_key_pem       = tls_private_key.dashy_signing.private_key_pem
...
resource "authentik_certificate_key_pair" "dashy_signing" {
  name             = "${var.application_slug}-oidc-signing"
  certificate_data = tls_self_signed_cert.dashy_signing.cert_pem
  key_data         = tls_private_key.dashy_signing.private_key_pem
}
...
resource "authentik_provider_oauth2" "dashy" {
  ...
  signing_key               = authentik_certificate_key_pair.dashy_signing.id
