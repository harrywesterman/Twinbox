...
export function summarizeProxmoxClusterResources(resources) {
  ...
  return {
    nodes: nodes
      ...
      .sort((left, right) => String(left?.node || left?.name || left?.id || "").localeCompare(
        String(right?.node || right?.name || right?.id || ""),
        undefined,
        { sensitivity: 'base' },
      )),
    summary,
...
function buildBaseline(stepInputs) {
  ...
  const memoryMb = Math.max(512, roundToStep(getInputDefault(stepInputs, 'memory_mb', 4096), 512));
  const diskGb = Math.max(10, roundToInteger(getInputDefault(stepInputs, 'disk_gb', 20)));
  ...
  return {
    ...
    memoryMb,
    diskGb,
    totalCpuCores: controlplaneCount * cpuCores + workerCount * cpuCores,
    totalMemoryMb: nodeCount * memoryMb,
    totalDiskGb: nodeCount * diskGb,
    bounds: {
      ...
      disk_gb: getInputBounds(stepInputs, 'disk_gb', { min: 10, max: 8192 }),
    },
  };
}
...
function deriveCapacityMultiplier(baseline, resources) {
  ...
  const memoryMultiplier = summary.freeMemoryMb > 0 ? summary.freeMemoryMb / Math.max(1, baseline.totalMemoryMb) : 0.5;
  const diskMultiplier = summary.freeDiskGb > 0 ? summary.freeDiskGb / Math.max(1, baseline.totalDiskGb) : 0.5;
...
function roundDiskOrFallback(value, fallback, bounds) {
  const rounded = roundToInteger(value);
  if (Number.isFinite(bounds?.min) && rounded < bounds.min) return bounds.min;
  if (Number.isFinite(bounds?.max) && rounded > bounds.max) return bounds.max;
  return Number.isFinite(rounded) ? rounded : fallback;
...
export function buildScaledProvisionInputs(scalePercent, stepInputs, currentValues = {}, dirtyFields = new Set(), resources = null) {
  ...
  const targetMemoryMb = roundMemoryOrFallback(
    baseline.memoryMb * sizeMultiplier,
    baseline.memoryMb,
    baseline.bounds.memory_mb,
  );
  const targetDiskGb = roundDiskOrFallback(
    baseline.diskGb * sizeMultiplier,
    baseline.diskGb,
    baseline.bounds.disk_gb,
  );
  ...
  return {
    ...
    memory_mb: isDirty('memory_mb')
      ? roundMemoryOrFallback(existing.memory_mb ?? baseline.memoryMb, baseline.memoryMb, baseline.bounds.memory_mb)
      : targetMemoryMb,
    disk_gb: isDirty('disk_gb')
      ? roundDiskOrFallback(existing.disk_gb ?? baseline.diskGb, baseline.diskGb, baseline.bounds.disk_gb)
      : targetDiskGb,
  };
}
...
export function buildProvisionScaleSummary(scalePercent, stepInputs, currentValues = {}, resources = null) {
  ...
  const memoryMb = Math.max(512, roundToStep(toNumber(currentValues.memory_mb, baseline.memoryMb), 512));
  const diskGb = Math.max(10, roundToInteger(toNumber(currentValues.disk_gb, baseline.diskGb)));
  const totalNodes = Math.max(1, controlplaneCount + workerCount);
  const totalCpuCores = totalNodes * cpuCores;
  const totalMemoryMb = totalNodes * memoryMb;
  const totalDiskGb = totalNodes * diskGb;
  ...
  return {
    ...
    memory_share_percent: resourceSummary?.freeMemoryMb > 0
      ? Math.round((totalMemoryMb / resourceSummary.freeMemoryMb) * 100)
      : null,
    disk_share_percent: resourceSummary?.freeDiskGb > 0
      ? Math.round((totalDiskGb / resourceSummary.freeDiskGb) * 100)
      : null,
...
export function buildProvisionHostCards(resources) {
  ...
  return summary.nodes.map((entry, index) => {
    const name = String(entry?.node || entry?.name || entry?.id || `host-${index + 1}`);
...
export function buildProvisionVmPlan(stepInputs, currentValues = {}) {
  ...
  const memoryMb = Math.max(512, roundToStep(toNumber(values.memory_mb, baseline.memoryMb), 512));
  const diskGb = Math.max(10, roundToInteger(toNumber(values.disk_gb, baseline.diskGb)));
...
function chooseBestHostForVm(vm, hostCards) {
  ...
  for (const host of hostCards) {
    ...
    const score = Math.min(cpuScore, memoryScore, diskScore);
...
export function suggestVmNodeMap(vmPlan, hostCards, currentMap = {}) {
  ...
  for (const vm of plan) {
    const preservedHost = typeof currentMap?.[vm.name] === 'string' ? currentMap[vm.name] : '';
...
export function buildProvisionPlacementBoard(stepInputs, currentValues = {}, resources = null) {
  ...
  for (const vm of vmPlan) {
    ...
    placementsByHost.get(hostId || 'Unassigned').push(placement);
  }
