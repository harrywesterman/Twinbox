...
const QUESTION_STEP_DEFS = [
  ...
  },
  {
    ...
    explanation:
      "Use an existing external S3-compatible service or let Twinbox provision a dedicated SeaweedFS backup VM. External credentials are stored as cluster secrets and are never written to job state or logs.",
    side_help:
      "External S3 provides off-site protection. A managed SeaweedFS VM is local backup storage and does not protect against failure of the complete Proxmox environment.",
    inputs: [
      {
  ...
  {
    id: "configure-proxmox-backup-server",
    title: "Configure Proxmox Backup Server",
    type: "config",
    journey_stage: "setup",
    summary: "Choose the host, storage, resources and address for the PBS VM.",
    explanation:
      "Twinbox provisions PBS as a separate Debian VM. Choose where it runs, how much local cache it gets, and review the free IP proposal before installation.",
    side_help:
      "The PBS cache is local and temporary; backup data is stored durably in the cluster S3 profile. Host and datastore choices are checked again immediately before the VM is created.",
    inputs: [
      {
        id: "pbs_node",
        label: "PBS Proxmox host",
        type: "string",
        required: true,
        help: "Online Proxmox host for the dedicated PBS VM.",
      },
      {
        id: "pbs_datastore",
        label: "PBS system datastore",
        type: "string",
        required: true,
        help: "Active VM datastore for the PBS system disk.",
      },
      {
        id: "pbs_cache_datastore",
        label: "PBS cache datastore",
        type: "string",
        required: true,
        help: "Active VM datastore for the persistent PBS cache disk.",
      },
      {
        id: "pbs_cpu",
        label: "PBS CPU (vCPU)",
        ...
        min: 2,
        default: 4,
        help: "PBS virtual CPUs.",
      },
      {
        id: "pbs_memory_gb",
        label: "PBS memory (GiB)",
        type: "integer",
        required: true,
        min: 4,
        default: 8,
        help: "PBS virtual memory.",
      },
      {
        id: "pbs_system_disk_gb",
        label: "PBS system disk (GiB)",
        type: "integer",
        required: true,
        min: 32,
        default: 32,
        help: "PBS operating system disk.",
      },
      {
        ...
        min: 64,
        default: 128,
        help: "Dedicated local cache disk; PBS must never fall back to the system disk.",
      },
      {
        id: "pbs_ip",
        label: "PBS VM IP",
        type: "ipv4",
        required: true,
        help: "Free static address proposed from the discovered cluster subnet.",
      },
    ],
  },
  {
