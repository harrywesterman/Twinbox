...
const webPublicIconsDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../public/assets/step-icons');
const portalPublicIconsDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../portal/public/assets/step-icons');
...
test('step icon manifest keeps the wizard icon set complete and paired', async () => {
  ...
  for (const entry of STEP_ICON_MANIFEST) {
    ...
    assert.equal(entry.sourceKind, 'local-svg', `expected ${entry.stepId} to be pinned as local svg`);
    assert.ok(!seen.has(entry.stepId), `expected unique step ids, got ${entry.stepId} twice`);
    seen.add(entry.stepId);
    assert.equal(entry.sourceFile, `./${entry.fileBase}.svg`, `expected ${entry.stepId} to resolve to local svg path`);

    if (entry.isAppOrPlatform) {
      assert.ok(entry.officialSourceType, `expected ${entry.stepId} app/platform icon to define officialSourceType`);
      assert.ok(entry.officialSourceUrl, `expected ${entry.stepId} app/platform icon to define officialSourceUrl`);
      assert.match(entry.officialSourceUrl, /^https?:\/\//, `expected ${entry.stepId} officialSourceUrl to be absolute`);
    }
    ...
    assert.doesNotMatch(svgRootMatch[0], /\swidth\s*=/i, `expected ${entry.fileBase}.svg root to omit width attr`);
    assert.doesNotMatch(svgRootMatch[0], /\sheight\s*=/i, `expected ${entry.fileBase}.svg root to omit height attr`);
    assert.equal(webPublicSvgText, svgText, `expected manager-web public ${entry.fileBase}.svg to match the source asset`);
    assert.equal(portalPublicSvgText, svgText, `expected portal public ${entry.fileBase}.svg to match the source asset`);
