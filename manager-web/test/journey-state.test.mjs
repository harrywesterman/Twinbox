...
function buildCatalog(stepStatuses = {}) {
  const coreSetupSteps = [
    ...
    [
      ...
      "Install CloudNativePG",
      { dependsOn: ["install-argocd", "install-longhorn-storage"] },
    ],
    [
    ...
    [
      ...
      "Create Users and Groups",
      { dependsOn: ["install-authentik-idp"] },
    ],
    ...
    [
      "install-loki",
      "Install Loki",
      { dependsOn: ["install-prometheus", "install-longhorn-storage"] },
    ],
    ...
    [
      ...
      "Install Management consoles",
      { dependsOn: ["install-dashy-dashboard", "install-twinbox-portal"] },
    ],
    [
      "install-pgadmin4",
      "Install pgAdmin 4",
...
test("wizard model exposes a linear setup rail and guided actions", () => {
  ...
  assert.equal(
    ...
    "[2026-03-20T10:10:00Z] Applying OpenTofu cluster plan"
  );
});
...
test("wizard model switches to manage mode when setup flow is complete", () => {
  ...
  completedCatalog.categories[0].steps = completedCatalog.categories[0].steps.map((step) =>
    step.id === "choose-ingress-route" ? { ...step, status: "configured" } : step
  );
