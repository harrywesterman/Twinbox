...
test("higher scale percentages grow the VM footprint and totals", () => {
  ...
  const summary = buildProvisionScaleSummary(
    values.scale_percent,
    stepInputs,
    values,
    largeClusterResources
  );
...
test("manual overrides stay in place when the scale slider changes", () => {
  ...
  const values = buildScaledProvisionInputs(
    100,
    stepInputs,
    {
      cpu_cores: 8,
...
test("placement board keeps the management VM fixed on its host and exposes its size", () => {
  ...
  assert.equal(
    host.assignments.find((vm) => vm.isFixed && vm.name === "twinbox-demo-mgt").disk_gb,
    60
  );
  assert.equal(
    host.assignments.find((vm) => vm.isFixed && vm.name === "twinbox-demo-mgt").memory_mb,
    8192
  );
});
...
test("placement board sorts hosts alphabetically by name", () => {
  ...
  assert.deepEqual(
    board.hostCards.map((host) => host.name),
    ["proxmox", "pve1", "pve2"]
  );
});
...
test("placement board preserves manual VM host assignments", () => {
  ...
  assert.equal(
    board.hostCards.find((host) => host.id === "pve-b").assignments[0].assignmentSource,
    "user-selected"
  );
  assert.equal(
    board.hostCards.find((host) => host.id === "pve-a").assignments[0].assignmentSource,
    "user-selected"
  );
});
...
test("placement board keeps manual placements while resuggesting the rest", () => {
  ...
  const remixedBoard = buildProvisionPlacementBoard(
    stepInputs,
    {
      vm_node_map: {
        "cp-1": "pve-b",
  ...
  assert.equal(
    remixedBoard.hostCards
      .find((host) => host.id === "pve-b")
      .assignments.some((vm) => vm.name === "cp-1"),
    true
  );
  assert.equal(
    remixedBoard.hostCards
...
test("automatic placement uses host free disk instead of a fixed 100GB worker size", () => {
  ...
  assert.equal(
    board.hostCards
      .find((host) => host.id === "pve1")
      .assignments.some((vm) => vm.isFixed && vm.name === "twinbox-prd-mgt"),
    true
  );
...
test("automatic placement replaces stale manual placements with a fresh suggestion", () => {
  ...
  const rerun = buildAutomaticProvisionPlacementResult(
    stepInputs,
    {
      vm_node_map: {
        "cp-1": "pve-b",
